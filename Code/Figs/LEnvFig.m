...
%     ch=loglog(1./srange,10*SNRrunAve(1./srange,Wp,Wm,0.5,w),'r:','LineWidth',1.5);
    ch=loglog(1./srange,10*SNRrunAve(1./srange,Wp,Wm,0.5,w),'b--','LineWidth',1.5);
end
epsvals=0.1.^(1:0.5:2);
for eps=epsvals
    q=ones(1,11);
...
legend([eh ch sh],{'Numeric Envelope','Shorter chains','Sticky end states'},'Location','Best');
clear nrange eh sh ch Wp Wm w;
clear epsvals eps q;