function AreaCoeffDeltaEtaPertGUI(varargin)

persistent p
if isempty(p)
    p=inputParser;
    p.FunctionName='AreaCoeffDeltaEtaPertGUI';
    p.StructExpand=true;
    p.KeepUnmatched=false;
    p.addParameter('nsteps',20,@(x)validateattributes(x,{'numeric'},{'scalar','integer'},'AreaCoeffDeltaEtaPertGUI','nsteps'));
    p.addParameter('nstates',20,@(x)validateattributes(x,{'numeric'},{'scalar','integer'},'AreaCoeffDeltaEtaPertGUI','nstates'));
end
p.parse(varargin{:});
...
%  Utility functions
    ...
    function MakePlot
        ...
        AreaCoeffPertPlot(Wp,Wm,S.dopertp*dWp,S.dopertm*dWm,qvec(end),pertvals,'Parent',axes1);
        xlabel(axes1,'Size of perturbation');
        ylabel(axes1,'Area Coefficients');
        title(axes1,'Effect of perturbation');
        legend(axes1,int2str((1:2*nstates)'))
        %update mixing time plot
        DeltaEtaPertPlot(Wp,Wm,S.dopertp*dWp,S.dopertm*dWm,qvec(end),w,pertvals,'Parent',axes2);
        xlabel(axes2,'Size of perturbation');
        ylabel(axes2,'Partial mixing times to + states');
        title(axes2,'Effect of perturbation');
        legend(axes2,int2str((1:2*nstates)'))
