function DiffAreaPertGUI(varargin)

persistent p
if isempty(p)
    p=inputParser;
    p.FunctionName='DiffAreaPertGUI';
    p.StructExpand=true;
    p.KeepUnmatched=false;
    p.addParameter('nsteps',20,@(x)validateattributes(x,{'numeric'},{'scalar','integer'},'DiffAreaPertGUI','nsteps'));
    p.addParameter('nstates',20,@(x)validateattributes(x,{'numeric'},{'scalar','integer'},'DiffAreaPertGUI','nstates'));
end
p.parse(varargin{:});
...
%  Utility functions
    ...
    function MakePlot
        ...
        DiffAreaPertPlot(Wp,Wm,S.dopertp*dWp,S.dopertm*dWm,qvec(end),w,pertvals,'Parent',axes2);
