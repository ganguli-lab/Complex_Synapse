...
if isempty(p)
    p=inputParser;
    p.FunctionName='ScaledDerivPertGUI';
    p.StructExpand=true;
    p.KeepUnmatched=false;
    p.addParameter('nsteps',20,@(x)validateattributes(x,{'numeric'},{'scalar','integer'},'ScaledDerivPertGUI','nsteps'));
    p.addParameter('nstates',20,@(x)validateattributes(x,{'numeric'},{'scalar','integer'},'ScaledDerivPertGUI','nstates'));
end
p.parse(varargin{:});
