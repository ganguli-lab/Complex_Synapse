function [ newmodel ] = BWrunGUI( truemodel )
...
%         ylabel(statepr,'State','FontSize',AxFontSize);
    ...
    function Simulate(~,~)
        simobj=truemodel.Simulate(S.fp,rand(2,S.num_t));
        PlotSim;
        init_obs=zeros(size(truemodel.Initial));
        init_obs(simobj.stateseq(1))=1;
        temptruemodel=SynapseIdModel(truemodel,'Initial',init_obs);
%         temptruemodel=temptruemodel.Normalise;
        PlotModel(temptruemodel,ax_true);
        %InitRand;
        %UpdateMets;
    ...
    function mets=CalcMets
        if ~isempty(simobj.potdep)
            mets(1,1)=HMMloglike(temptruemodel,simobj);
            mets(2,1)=HMMloglike(newmodel,simobj);
        else
            mets(1:2,1)=NaN(2,1);
        end
        [mets(1,4),mets(1,2:3)]=temptruemodel.KLdivs(newmodel);
