function [ M_new,init_new ] = RJupdateGUI( M,init,w )
...
%         ylabel(statepr,'State','FontSize',AxFontSize);
    ...
    function Simulate(~,~)
        chunks=[S.num_t*(0:S.num_ch-1)+1;S.num_t*(1:S.num_ch)];
        [st,pd]=StateSequence(init,M,rand(2,S.num_ch*S.num_t),S.fp);
        rd=(3-w(st)')/2;
        PlotSim;
...
%         PlotStatePr;
    ...
    function Update(~,~)
       if ~isempty(rd)
        M_prev=M_new;
        init_prev=init_new;
        [M_new,init_new,stpr,like]=RJupdate(chunks,rd,init_prev,outProj,M_prev,pd);
        [M_new,~,ix]=SortByWtEta(M_new,w,S.fp);
        init_new=init_new(ix);
        UpdateMets;
        PlotStatePr;
