...
if isempty(p)
    p=inputParser;
    p.FunctionName='DoubleEnv_Case4Exp2';
    p.StructExpand=true;
    p.KeepUnmatched=true;
    p.addRequired('t1',@(x)validateattributes(x,{'numeric'},{'scalar','positive'},'DoubleEnv_Case4Exp2','t1',1));
    p.addRequired('t2',@(x)validateattributes(x,{'numeric'},{'scalar','positive'},'DoubleEnv_Case4Exp2','t2',2));
    p.addRequired('S2',@(x)validateattributes(x,{'numeric'},{'scalar','positive'},'DoubleEnv_Case4Exp2','S2',3));
    p.addRequired('n',@(x)validateattributes(x,{'numeric'},{'scalar','positive'},'DoubleEnv_Case4Exp2','n',4));
    p.addParameter('t',[],@(x) validateattributes(x,{'numeric'},{},'DoubleEnv_Case4Exp2','t'));
    p.addParameter('Constraint3',true,@(x) validateattributes(x,{'logical'},{'scalar'},'DoubleEnv_Case4Exp2','Constraint3'));
    p.addParameter('Display','off',@(x) parsevalidatestring(x,{'on','off'},'DoubleEnv_Case4Exp2','Display'));
    p.addParameter('Jacobian','on',@(x) parsevalidatestring(x,{'on','off'},'DoubleEnv_Case4Exp2','Jacobian'));
end
