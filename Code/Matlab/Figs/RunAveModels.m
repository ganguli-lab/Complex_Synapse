function [ fig ] = RunAveModels( srange,M,sqrtNum,chains,inds )
%RUNAVEPROVENNUM Summary of this function goes here
...
for i=inds
    loglog(ax,tau,(sqrtNum)*chains(i).snrb,'b','LineWidth',2);
end
...
% 
%         annotation('textbox',...
%             dsxy2figxy(ax,[tau(toend) 0.1*area(toend) tau(end) sqrt(area(toend)*5*sqrtNum)]),...
%             'String','$\frac{\sqrt{\mathsf{N}}(\mathsf{M-1})}{\mathsf{r\tau}}$',...
%             'Interpreter','latex','FontSize',22,...
%             'LineStyle','none','VerticalAlignment','bottom','HorizontalAlignment','center');
% %            dsxy2figxy(ax,[tau(toend) area(toend) tau(end) sqrt(area(toend)*5*sqrtNum)]),...
% 
%         annotation('textbox',...
%             dsxy2figxy(ax,[0.5*(M-1) 0.9*sqrtNum 2*(M-1) 1.6*sqrtNum]),...
