...
%         AreaPertPlot(Wp,Wm,S.dopertp*dWp,S.dopertm*dWm,qvec(end),w,pertvals,'Parent',axes1);
        m1vals=(0:1/nsteps:1)';
        stability=(sqrt(4*qvec(4)*qvec(5)+(qvec(2)+2*qvec(5))^2)-qvec(2)-2*qvec(5))/2;
        A1=(m1vals * (qvec(2) + 3 * qvec(4)) +  5 * qvec(4) * qvec(5))./(m1vals * (qvec(2) + qvec(4)) +  qvec(4) * qvec(5));
        A2=(3 * m1vals + 3 * qvec(2) +  5 * qvec(5))./(m1vals + qvec(2) + qvec(5));
        plot(m1vals,A1,'b',m1vals,A2,'g','Parent',axes1);
        line(stability*[1 1],ylim(axes1),'Color','r');
        xlabel(axes1,'Transition rate 1 to 2');
