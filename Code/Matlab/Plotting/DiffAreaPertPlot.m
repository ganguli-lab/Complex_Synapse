function [ h ] = DiffAreaPertPlot( Wp, Wm, Pertp, Pertm, fp, w, pertvals, varargin )
%H=AREAPERTPLOT(WP,WM,PERTP,PERTM,FP,PERTVALS,...) plot of area under SNR
...
[prow,pcol]=ind2sub(size(Pertp),find(Pertp==1));
[mrow,mcol]=ind2sub(size(Pertm),find(Pertm==1));

for i=1:numel(pertvals)
    darea(i)= DiffArea(Wp + pertvals(i) * Pertp, Wm + pertvals(i) * Pertm, fp, w, prow,pcol,1)...
        + DiffArea(Wp + pertvals(i) * Pertp, Wm + pertvals(i) * Pertm, fp, w, mrow,mcol,-1);
end
