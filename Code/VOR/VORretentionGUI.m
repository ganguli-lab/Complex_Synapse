function VORretentionGUI(varargin)
...
slparams={'paramPot','paramWT','paramKO','fpNorm','fpInc'};
...
%  Utility functions
    ...
    function MakeSlider(which,varname)
        ...
        set(phs(which),'Title',varname,'FontSize',FontSize);
        %slider
    ...
    function MakeEditBox(which,varname)
        ...
        set(phe(which),'Title',varname,'FontSize',FontSize);
        %control
...
%             'Position',[x y 1/6 1/2]);
    ...
    function MakePlot
        vorexptobj=VORretentionBuilder(S.builder_h,S.numStates,S.paramPot,S.paramWT,S.paramKO,S.fpNorm,S.fpInc,S.t_inc,S.t_dec);
        vorexptobj.PlotLearnS('Parent',axes1,'LineWidth',2,varargin{:});
