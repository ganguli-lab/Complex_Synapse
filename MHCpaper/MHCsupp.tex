...
%}
\date{}
\DeclareCaptionLabelFormat{supp}{\textbf{#1 S#2}}
...
\begin{myenuma}
  \begin{tabular}{l@{\hspace{0.05\linewidth}}l@{\hspace{0.05\linewidth}}l}
      ...
      \\[1.5cm]
      \aligntop{\includegraphics[width=0.3\linewidth]{serial_med_learnS.eps}}
      &
...
\begin{aligned}
  \Phi &= \frac{q\pot q\dep \Delta f}{f\pot q\pot + f\dep q\dep}, &\qquad
  \pdiff{\Phi}{q\dep} &= \frac{f\pot (q\pot)^2 \Delta f}{(f\pot q\pot + f\dep q\dep)^2}, \\ &&
  \pdiff{\Phi}{f\pot} &= \frac{(f\pot\inc q\pot + f\dep\inc q\dep) q\pot q\dep}{(f\pot q\pot + f\dep q\dep)^2}.
\end{aligned}
\end{equation}
%
The flux is a monotonically increasing function of $q\dep$ and $f\pot$.
Therefore, enhancing plasticity in this way or pre-training can only enhance the initial learning rate, unlike the experimental results shown in \modelfig.
...
\end{equation}
%
So, we see that pre-training will speed up learning for all parameter values, unlike what is seen in the experiment.
This can be seen in
...
\end{equation}
%
which is always larger than \eqref{eq:binWTnopre}, unlike what is seen in the experiments (see \modelfig).
...
One can see that pre-training will always increase the fraction of synapses available for depression.
...
Therefore, enhancing plasticity in this way can only enhance the initial learning rate, for all values the parameters.
...
Therefore, pre-training can can only enhance the initial learning rate, for all values the parameters.
...
\autoref{fig:sim_results}\ref{fig:cascade_sim}. %\autoref{fig:cascade_short} and \autoref{fig:cascade_long}.
It is difficult to study this model analytically.
However, the numerical results can helps us understand it qualitatively.
...
This effect is weaker for the \KO\ models, as the equilibrium distribution is not as heavily concentrated at the end, as the distribution before pre-training is shifted in the other direction.
...
\begin{figure}
 ...
 \begin{myenuma}
  ...
  \begin{tabularn}{|l|c|c|c|c|c|c|c|c|c|}
    ...
    \cline{1-10}
    Serial   & 10 & $0.12$  & $0.14$  & $0.2$  & 0.5 & 0.89 & 0.11 & 5  & 100  &\label{tr:serial_fit} \\
    Cascade  & 14 & $0.386$  & $0.398$  & $0.466$  & 0.522 & 0.63 & 0.002 & 1.5  & 200  &\label{tr:cascade_fit} \\
    Non-uni. & 12 & $0.4$    & $0.4$    & $0.53$   & 0.5 & 0.7 & 0.1 & 5  & 500  &\label{tr:nonuni_fit} \\
    \cline{1-10}
  \end{tabularn}}
