...
\resultsfig{step-multistate}{moderate training}{($\Delta f=-0.17$)}{multistate_med}

\resultsfig{step-multistate}{strong training}{($\Delta f=-0.4$)}{multistate_strong}
...
\resultsfig{pooled resource}{heavy depletion}{($q\lmin/q\lmax=0.25$)}{pooled_scarce}
...
\resultsfig{cascade}{short pre-training}{($rt_\text{pre}=50$)}{cascade_short}

\resultsfig{cascade}{long pre-training}{($rt_\text{pre}=150$)}{cascade_long}
