...
For the serial, multistate and two state models, we will use the same value for the transition probabilities, $q$, for potentiation and depression in the wild-type as well as potentiation in the \KO\ mutant.
...
We will use a larger value for depression in the \KO\ mutant.
...
We use the same values for wild-type and mutant for the reasons discussed above.
...
\begin{table}
 \begin{center}
  \begin{tabularn}{|l|c|c|c|c|c|c|}
    ...
    Pooled        & 7  & $q=0.008$        & $q\in[0.0006,0.6]$  & $q\in[0.001,1]$
    ...
    Non-uniform MS& 10 & $x=0.25$ & $x=0.25$ & $x=0.33$ & 0.3  & 150 &\label{tr:nonuni} \\
    \cline{1-7}
%   \hline
%    % after \\: \hline or \cline{col1-col2} \cline{col3-col4} ...
...
\newcommand{\captiontext}[1]{ can be found in \autoref{tr:#1} of \autoref{tab:params}.
  ...
  (\ref{fig:#1_pr}) Evolution of probability distributions for (\ref{fig:#1_pr_WT_nopre},\ref{fig:#1_pr_WT_pre}) wild-type and  (\ref{fig:#1_pr_KO_nopre},\ref{fig:#1_pr_KO_pre}) \KO\ mutant without (\ref{fig:#1_pr_WT_nopre},\ref{fig:#1_pr_KO_nopre}) and with (\ref{fig:#1_pr_WT_pre},\ref{fig:#1_pr_KO_pre}) pre-training,
...
Now, consider the mutant, for which we define $\beta=q\pot/q\dep<1$, and we set $q\pot=q$.
Without pre-training:
%
...
As we would like pre-training to slow down learning in the wild-type but speed it up in the mutant, it would seem that we require $\Delta f^*(1,M) < \Delta f < \Delta f^*(\beta,M)$ (remembering that the wild-type corresponds to $\beta=1$).
...
Let's look at \autoref{fig:multistate_strong}\ref{fig:multistate_strong_learnS}, for which $\Delta f > \Delta f^*(\beta,M)$, so that pre-training initially slows down learning for both wild-type and mutant.
...
The subleading effects (the normalisation of the probabilities) are responsible for the faster learning in the mutant.
...
Intuitively, this model is missing two features of the \hyperref[sec:multistate]{multistate model}.
First, it does not have the exponential amplification of the effect of initial saturation bias as this model does not have a chain of states for the distribution to decay across.
...
Like the \hyperref[sec:binary]{two state model}, this model is missing two features of the \hyperref[sec:multistate]{multistate model}.
...
\resultsfig{pooled resource}{unaffected potentiation}{($q\pot\lmax=q\pot\lmin$)}{pooled_deponly}
...
Gain-decrease pre-training will shift the distribution to the potentiated side, where there are more synapses available for depression and resources are more plentiful, for both the mutant and wild type (see \autoref{fig:pooled_scarce}\ref{fig:pooled_scarce_eq}, red traces).
...
The situation discussed in the main text, with resource depletion for depression only and potentiating transitions  unaffected, is shown in \autoref{fig:pooled_deponly}.
It shows the same learning patterns as the more symmetric versions.

This model shares one feature of the \hyperref[sec:multistate]{multistate model}.
...
This model shares both features of the \hyperref[sec:multistate]{multistate model}.
...
First, we see that the mutant is slower than wild-type without pre-training, as seen experimentally.
...
The wild-type will be pushed further that the mutant, as the latter started further to the weaker side as explained above (compare \autoref{fig:nonuni}\ref{fig:nonuni_eq}\ref{fig:nonuni_eq_WT} and \ref{fig:nonuni_eq_KO}).
As the depressing transitions have lower probability for the strongest states (see \autoref{fig:nonuni_model}), this, in addition to the enhanced depression of the mutant, will result in the mutant learning faster than wild-type, as seen experimentally.
...
The second effect will be weaker in the mutant than in wild-type, as they started further to the weaker side as explained above.
This means that, for appropriate parameter choices, gain-decrease pretraining can speed up learning in the mutant, but also slow down learning in wild-type, as seen experimentally.

Like the \hyperref[sec:cascade]{cascade model}, this model also shares both features of the \hyperref[sec:multistate]{multistate model}.
First, the effect of initial saturation bias is amplified by the exponential decay of transition probabilities away from the middle.
Second, it has the metaplastic effect where repeated potentiation makes future depression harder, also due to he exponential decay of transition probabilities.
