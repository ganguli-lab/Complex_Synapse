...
This is because the effects of the mutation is well localised to the Purkinje cells, so the activity of the parallel and climbing fibres should not change very much.
...
\begin{table}
 \begin{center}
  \begin{tabularn}{|l|c|c|c|c|c|c|}
    ...
    \cline{1-7}
    Step-multistate    & 10 & $q=0.3$  & $q=0.4$  & 0.1  & 5 & 20  &\label{tr:multistate_weak} \\
    Step-multistate    & 10 & $q=0.3$  & $q=0.4$  & 0.3  & 5 & 20  &\label{tr:multistate_med} \\
    Step-multistate    & 10 & $q=0.3$  & $q=0.4$  & 0.45 & 5 & 30  &\label{tr:multistate_strong} \\
    Two-state          & 2  & $q=0.1$  & $q=0.2$  & 0.1  & 5 & 5   &\label{tr:binary} \\
    Linear Multistate  & 10 & $q=0.3$  & $q=0.4$  & 0.3  & 5 & 5   &\label{tr:multistate_lin} \\
...
\resultsfig{step-multistate}{weak training}{($\Delta f=-0.1$)}{multistate_weak}

\resultsfig{step-multistate}{moderate training}{($\Delta f=-0.3$)}{multistate_med}

\resultsfig{step-multistate}{strong training}{($\Delta f=-0.45$)}{multistate_strong}
...
\resultsfign{linear multistate model}{multistate_lin}
