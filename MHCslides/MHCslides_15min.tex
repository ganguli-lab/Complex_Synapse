...
%
 ...
 \vp Claim: due to basal activity \lto\ biased synaptic population \\
 \lto\ fewer synapses available for learning.

 \vp Analysis of models of complex synapses:\\ motor learning of enhanced LTD mice \lto\ constrain synaptic structure.
%
\end{frame}
...
\begin{frame}{MHC-I \texorpdfstring{D$^\mathrm{b}$K$^\mathrm{b} -/-$}{DbKb -/-} knockout}
%
...
%
 ...
 \note[item]{Many expt checks of this, but we'll focus on one...}
%
\end{frame}
...
%
 ...
 \note[item]{Diagonal comparisons: parameter fitting. Depend on size of KO vs.\ pretraining}
...
%
 \begin{itemize}
   \item Can the saturation effect overcome faster depression?
...
%  There are $N$ identical synapses with $M$ internal functional states.
  %
  \begin{center}
    \only<1>{\includegraphics[width=0.9\linewidth]{synapse_model_1.svg}}
    \only<2>{\includegraphics[width=0.9\linewidth]{synapse_model_2.svg}}
  \end{center}
  %
  ...
  \note[item]{Readout: synaptic weight vec when in each state.}
    \note<2>[item]{Mutation: lower threshold \lto\ increase transition probs}
    \note<2>[item]{Training: Changes statistics of LTP/LTD. Only parameters we have. Don't care about $r$.}
    \note<2>[item]{Learning: Only output we have. Don't keep track of synaptic identity.}
...
%
 ...
 \end{tabular}
 \end{center}
 \note[item]{Binary fails}
 \note[item]{KO: faster depression wins over bias}
 \note[item]{pre: reduces/reverses bias. always helps.}
...
\begin{frame}{Initial distributions}
%
 %
 \begin{center}
   \alignmid{\includegraphics[height=0.3\linewidth]{binary_bar_ko_wo_s.svg}} \hspace{0.1\linewidth}
   \only<1>{\alignmid{\includegraphics[height=0.3\linewidth]{serial_bar_ko_wo_s.svg}}}
   \only<2>{\alignmid{\includegraphics[height=0.3\linewidth]{serial_bar_ko_w_s.svg}}}
   \only<3>{\alignmid{\includegraphics[height=0.3\linewidth]{serial_bar_wt_w_s.svg}}}
 \end{center}
 ...
 \note<2->[item]{Pretraining: little repopulates boundary}
 \note<3>[item]{Too much pushes to other side, depopulates boundary}
 %
%
\end{frame}
...
%
 ...
 \end{itemize}
 \note<2>[item]{borne out by other models that fail/succeed}

 \visible<2>{
 \begin{center}
 \parbox[t]{0.3\linewidth}{\centering
    Fail:\\
    \vp\aligntop{\includegraphics[width=0.9\linewidth]{multistate_s.svg}}\\
    \vp\aligntop{\includegraphics[width=0.9\linewidth]{pooled_s.svg}}
 }
 \hp
 \parbox[t]{0.3\linewidth}{\centering
    Succeed:\\
    \vp\aligntop{\includegraphics[width=0.9\linewidth]{multistate_nonuni_s.svg}}\\
    \vp\aligntop{\includegraphics[width=0.9\linewidth]{cascade_s.svg}}
 }
 \end{center}
...
%   \note[item]{\eg moving away from weight boundary, or weaker transitions.}
   \item We can find a purely synaptic explanation of impaired learning,\\
    iff the synapses have two features.
   \note[item]{Other explanations? Non-linearity in PK cell?}
   \vp \item We used behaviour to constrain molecular structure of synapses!
   \vp \item Can we constrain it further with more experiments?
 \end{itemize}
%
\end{frame}
...
%
 ...
 \vp\textbf{Funding}: Swartz Founsation, Stanford Bio-X Genentech fellowship.
%
\end{frame}
