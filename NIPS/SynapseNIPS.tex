...
\end{equation}
%
The initial SNR is closely related to the flux from the states with $\w_i=-1$ to those with $\w_j=+1$:
%
...
First passage time theory introduced in the \hyperref[sec:order]{previous section} becomes useful because the area has a simple expression in terms of quantities introduced in \eqref{eq:kemenypm}:
%
...
The area of a linear chain model can be expressed directly in terms of its equilibrium state distribution, $\eq$, yielding the following upper bound on the area of any model with the same $\eq$:
%
...
Thus models that optimize area optimize memory at late times, at the expense of early times.

\begin{figure}[tbp]
 \begin{center}
  \includegraphics[width=0.35\linewidth]{multistate_sticky-generated.pdf}
 \end{center}
...
Nevertheless, we have proven rigorously that no model's memory curve can ever exceed this envelope, and that it is at least tight for late times, longer than $\CO(\frac{M}{r})$, where models of the form in \autoref{fig:max_area} can come close to the envelope.

\begin{figure}
 \begin{center}
 \begin{myenuma}
  \item\aligntop{\includegraphics[width=0.38\linewidth]{env.eps}}\label{fig:envplot}\hp
