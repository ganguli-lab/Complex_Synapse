...
\begin{aligned}
  \pdiff{\CL}{\M^\pm_{gh}} =&\,
    ...
    &+ \sum_{ab} F_{ab} (\eq\enc\uv^a) \vv^a_g (\uv^b_h-\uv^b_g) (\vv^a\w) \\
    &+ \mu^\pm_{gh} - \mu^\pm_{gg}.
\end{aligned}
\end{equation}
%
...
\begin{equation}\label{eq:levol}
  \mathbf{G}(s) = \int_0^{\infty}\dr t\, \e^{(\MM-s) t}.
\end{equation}
%
For $\Re s>0$, we have 
%
\begin{equation}\label{eq:levolcalc}
  (s-\MM)\mathbf{G}(s) = \int_0^{\infty}\dr t\, (s-\MM)\e^{(\MM-s) t} = \brk{-\e^{(\MM-s) t}}_0^{\infty} = \I.
\end{equation}
%
As $(s-\MM)$ is invertible for $\Re s>0$, because all eigenvalues of $\MM$ are nonpositive, we have 
%
\begin{equation}\label{eq:levolres}
  \mathbf{G}(s) = (s-\MM)\inv.
\end{equation}
%
For $s=0$, we can avoid our problems by replacing $\mathbf{G}(s) \to \fund(s)$.

Now consider the Laplace transform of the SNR curve \eqref{eq:SNRcurve}
%
\begin{equation}\label{eq:lsnr}
\begin{aligned}
  A(s) &= \int_0^{\infty}\dr t \e^{-st}\SNR(t) \\
   &= \int_0^{\infty}\dr t\, \sqrt{N}(2f^+f^-) \eq \enc \e^{(r\frg-s)t} \w \\
...
Thanks to \eqref{eq:alfund}, we can replace $\mathbf{G}(s) \to \fund(s)$, as $\enc\onev=0$.
%
...
\begin{equation}\label{eq:lAdiff}
  \pdiff{\hat{A}(s)}{\W^\pm_{gh}} = 2rf^\pm \eq_g \brk{ \sum_i a_i(s) (\fptb_{gi}(0)-\fptb_{hi}(0))
     +c_g(s) (\eta^+_g(s) - \eta^+_h(s)) } \pm 2\eq_g (\eta^+_g(s) - \eta^+_h(s)).
\end{equation}
%
This is exactly the same as \eqref{eq:derivA}.
It follows that, using exactly the same perturbations, that the Laplace transform of the SNR curve for \emph{any} model is lower than that of some model with the serial topology having the same equilibrium distribution.
