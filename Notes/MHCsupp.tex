...
\hypersetup{colorlinks,linkcolor=blue,citecolor=violet}
...
\begin{figure}
 ...
 \begin{myenuma}
  ...
  \item\aligntop{\includegraphics[height=2cm]{cascade.svg}}\label{fig:cascade_model}
 \end{myenuma}
 \end{center}
...
\begin{figure}
 \begin{center}
  \aligntop{\includegraphics[height=1.7cm]{pooled.svg}}
 \end{center}
...
Pre-training has two effects: it produces a slope in the right direction (see \autoref{fig:multistate_weak}\ref{fig:multistate_weak_eq}), but it also reduces the distribution at the centre (see \autoref{fig:multistate_strong}\ref{fig:multistate_strong_eq}).
...
However, in this case pre-training proceeds much faster in the mutant than wild-type (see \autoref{fig:multistate_med}\ref{fig:multistate_med_learn} and \autoref{fig:multistate_strong}\ref{fig:multistate_strong_learn}), which is \emph{not} seen in the experiment.
