...
\begin{table}
 \begin{center}
  \begin{tabularn}{|l|c|c|c|c|c|c|}
    ...
    OG Multistate & 10 & $q=0.6$  & $q=0.8$  & -0.3  & 20 & 20  &\label{tr:multistate_lin} \\
...
\newcommand{\resultsfig}[2]{\begin{figure}
 ...
 \end{center}
  ...
  with states of weak synaptic strength at the top and strong states underneath. } \label{fig:#1}
\end{figure}}
...
\resultsfig{multistate_med}{Simulation results for the multistate model with moderate training ($\Delta f=-0.17$)}
...
\resultsfig{multistate_lin}{Simulation results for multistate model with linear weights}%\label{fig:multistate_lin}
...
\resultsfig{pooled_plenty}{Simulation results for the pooled resource model with light depletion ($q\lmin/q\lmax=0.75$)}

\resultsfig{pooled_scarce}{Simulation results for the pooled resource model with heavy depletion ($q\lmin/q\lmax=0.25$)}
