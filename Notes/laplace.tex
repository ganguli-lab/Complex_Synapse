...
\begin{aligned}
  \left.\pdiff{\hat{A}(s)}{\varepsilon}\right|_{\varepsilon=0} &\propto
    m \prn{S(m\beta) - S((m-1)\beta)} + (m-1) S(m\beta) \prn{S(m\beta) + 1} = 0,\\
...
\begin{aligned}
  \left.\pdiff{\hat{A}(s)}{\varepsilon}\right|_{\varepsilon=0} \propto &\,
    m(2s+1) \sqrt{2s} \sinh\brk{{\prn{m-\tfrac{1}{2}}\beta}} 
    - S(m\beta) [S(m\beta) + 1]
      = 0,\\
  \left.\pdiff{\hat{A}(s)}{\varepsilon}\right|_{\varepsilon=1} \propto &\,
    (m-1) \sqrt{2s} \sinh\brk{{\prn{m-\tfrac{1}{2}}\beta}} 
    - (2s+1) S((m-1)\beta) [S((m-1)\beta) + 1]
      = 0,\\
    &\text{where}\quad
...
\begin{aligned}
  ...
  & \sqrt{N}\prn{ \frac{ 4\sinh^2 \frac{y_*}{2} }{ y_* \cosh y_* } }\sinh\inv \!\!\sqrt{1/2r\tau}, &
...
\end{equation}
%
where  we use the notation \( \Delta_{M,\varepsilon}[T(M, \beta)] = T(M, \beta) - \varepsilon T(M-2, \beta) \).
