...
\end{equation}
%
Therefore, provided that state \(i\) has nonzero equilibrium probability, degeneracy in \(\etw\) implies degeneracy in \(\thb\) for an otimal model.
...
\begin{equation*}
\begin{aligned}
  [\W\pot \etw]_i &= \sum_{j \in C_{a+1}} \Wm\pot_{ij} (\etwm_j - \etwm_i)
...
% (There is still an ambiguity concering the scale, but it makes sense to choose to saturate the diagonal constraints in \cref{eq:wdof} so that \cref{eq:optmodelscale} holds).
