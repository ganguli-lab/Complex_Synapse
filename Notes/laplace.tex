...
When the synapse is subjected to a plasticity event the internal state will change stochastically, described by the matrices $\M^\mu$ whose $i,j$ elements are the transition probabilities from state $i$ to state $j$, 
and \(\mu\) labels the diffrerent types of plasticity event.
Potentiating events have \(\mu=\)pot, depressing events have \(\mu=\)dep.
...
\end{equation*}
%
The matrices $\M^\mu$ are transition matrices of discrete time Markov processes.
Their elements must satisfy the following constraints
%
\begin{equation}\label{eq:constr}
  \M^\mu_{ij} \geq 0 
    \quad \forall \; \mu, i, j, \qquad
  \sum_j \M^\mu_{ij} = 1
    \quad \forall \; \mu, i.
\end{equation}
%
The upper bounds $\M^\mu_{ij} \leq 1$ follow automatically from these.
...
\end{equation}
%
If the probability of a given plasticity event being potentiating/depressing is $f^\mu$, the probability of the sequence above (conditioned on the number of events) is given by $f\pot f\dep f\dep f\pot f\dep f\pot f\pot f\dep \ldots f\pot$.
...
\begin{figure}[tb]
  ...
  \Cref{eq:finitelagrangian,eq:KTfinite,eq:finitederivs} are solved numericallly with $f\potdep=\frac{1}{2}$, the number of synapses $N=10^4$ and the number of states $M=12$.
