...
\begin{multline}\label{eq:plastseqtypes}
  \pr(t_n) = \pr(t_0) (f\pot\M\pot+f\dep\M\dep)  \ldots \\
    \ldots (f\pot\M\pot+f\dep\M\dep) 
    \M\potdep (f\pot\M\pot+f\dep\M\dep) \ldots \\
    \ldots (f\pot\M\pot+f\dep\M\dep),
\end{multline}
% 
...
The second choice does not leave \(\enc\) invariant (in fact \( \shift{\enc}(s) = \enc + (f\pot - f\dep) (\onev \eq - \I) \)), but because \( \eq (\onev \eq - \I) = 0 \) the shift does not make any contribution to \cref{eq:larea}.
Either way:
%
...
Each constraint we find will be a necessary condition for the existence of a set of Markov processes \( \M_{ij}^\nu \) that result in \( (\CI_a, \tau_a) \).
We can imagine the notion of a complete set of constraints: constraints that, when combined, are also sufficient for the existence of a \( \M_{ij}^\nu \).
...
\begin{equation}\label{eq:innereq}
  \av{\mathbf{u}, \mathbf{v}} = \sum_i \eq_i u_i^\ast v_i ,
  \qquad
...
When the process satisfies detailed balance, \ie \(\F_{ij}\) is symmetric, we can relate these inner products to quantities of interest.

We can write
%
\begin{equation*}
  \av{\mathbf{u}, -\MM\mathbf{v}} = \sum_{i \neq j} \eq_i u_i^\ast \MM_{ij} (v_i - v_j),
\end{equation*}
%
...
\begin{equation*}
  \av{\mathbf{u}, -\MM \mathbf{v}} 
      = \frac{1}{2}\sum_{i \neq j} \F_{ij} (u_i^\ast - u_j^\ast) (v_i - v_j) 
...
\begin{equation*}
  \CI(\mathbf{u}) = \frac{(\eq \enc \mathbf{u}) (\mathbf{u}^\dag \w)}{\mathbf{u}^\dag \mathbf{u}}
      = \frac{\sum_{i \neq j} \eq_i \enc_{ij} (u_j - u_i) \sum_k \eq_k u_k \w_k} 
             {\sum_{l} \eq_l u_l^2} .
\end{equation*}
%
...
\begin{equation*}
  ...
  r \eq \enc \mathbf{u} &= \intd[_{-\infty}^{\infty}]{\alpha}  \brk{
        \F\pot_{\CA_\alpha \CA_\alpha^c} - \F\dep_{\CA_\alpha \CA_\alpha^c} 
      - \F\pot_{\CA_\alpha^c \CA_\alpha} + \F\dep_{\CA_\alpha^c \CA_\alpha} }
    \\
    &= \intd[_{-\infty}^{\infty}]{\alpha}  \brk{
        \F_{\CA_\alpha \CA_\alpha^c} + \F_{\CA_\alpha^c \CA_\alpha} 
        - 2\F\dep_{\CA_\alpha \CA_\alpha^c} - 2\F\pot_{\CA_\alpha^c \CA_\alpha} }
  \end{aligned}
  %
  \quad \text{where} \quad
  %
...
\end{equation*}
%
In exactly the same way as the proof of \cref{eq:initbnd}, we can argue that the two terms with the negative signs are indeed non-positive as they do not involve the negative diagonal elements of~\( \F_{ij}^\nu \).
Therefore the other terms provide an upper bound.
%
\begin{equation*}
\begin{aligned}
  r \nrm{\mathbf{u}}^2 \CI(\mathbf{u}) 
      &\leq \intd[_{-\infty}^{\infty}]{\alpha}  \brk{
        \F_{\CA_\alpha \CA_\alpha^c} + \F_{\CA_\alpha^c \CA_\alpha} } 
        \av{\mathbf{u}, \w}
  ...
  \\  &= \sum_{i \neq j} \F_{ij} \abs{u_j - u_i} \av{\mathbf{u}, \w}
  \\  &= \aav{\abs{\boldsymbol{\Delta}\mathbf{u}}, \onem} \av{\mathbf{u}, \w} .
\end{aligned}
\end{equation*}
%
...
\end{equation*}
%
We also have \( \nrm{\w}^2 = \sum_i \eq_i \w_i^2 = 1 \).
Furthermore, using \cref{eq:constri}, \( \nnrm{\onem}^2 = \sum_{i \neq j} \eq_i \MM_{ij} \leq r \sum_i \eq_i = r\).
Then, remembering \cref{eq:numrange},
%
\begin{equation}\label{eq:sqrtnumrange}
  \sqrt{r} \CI(\mathbf{u}) \leq \sqrt{2\mathcal{W}(-\MM, \mathbf{u})}.
\end{equation}
%
