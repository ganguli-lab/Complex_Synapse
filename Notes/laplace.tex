...
\begin{aligned}
  \frac{1}{f^\nu} \pdiff{\CL}{\Mh^\nu_{mn}} = & \,
    \eq_m \prn{ r \brk{\thb_m(s) - \thb_n(s)}
     + \prn{r \mathbf{c}_m(s) + (-1)^\nu} \brk{\etw_m(s) - \etw_n(s)} } \\
     &+ \mu^\nu_{mn} -  \mu^\nu_{mm}
     + r \lambda \eq_m (\etw_{m}(0)-\etw_{n}(0)) 
    = 0.
\end{aligned}
\end{equation}
%
...
\begin{aligned}
  \mu^\nu_{mm} &= - \eq_m \prn{ r \alb^\nu_m(s) 
    + r \lambda \omb^\nu_i(0) 
    + \prn{r \mathbf{c}_m(s) + (-1)^\nu} \omb^\nu_m(s)
    }, \\
  \mu^\nu_{mn} &= \eq_m \big( 
      r \brk{ \thb_n(s) - \thb_m(s) - \alb^\nu_m(s) }
    + r \lambda \brk{ \etw_n(0) - \etw_m(0) - \omb^\nu_m(0) } 
  \\ &\phantom{= - \eq_m \big( }
    + \brk{rc_m(s) + (-1)^\nu} 
...
\begin{equation}\label{eq:shiftscale}
  \lambda {\circledast}_{s} \M \equiv (1 - \lambda) \I + \lambda \M^\nu
                + \frac{(\lambda - 1) s}{r} (\onev \eq - \I).
\end{equation}
%
Then we have
%
\begin{equation}\label{eq:lshiftscale}
  \hat{A} \prn{s; \M^\nu} 
    = \hat{A} \prn{0, \shift{\M}^\nu(s)}
    = \hat{A} \prn{0, \lambda * \shift{\M}^\nu(s)}
    = \hat{A} \prn{s, \lambda {\circledast}_{s} \M^\nu}.
\end{equation}
%
The problem here is that the lower bound, \(\M^\nu_{ij} \geq 0\), is not invariant under this transformation, unlike the \(s = 0\) case.
...
\begin{equation}\label{eq:lshiftconstr}
  \shift{\M}^\nu_{ij} \geq \frac{s \eq_j}{r},
    \quad \forall \; \nu, i, j : i \neq j, \qquad
  \sum_{j: j \neq i} \shift{\M}^\nu_{ij} \leq 1 + \frac{s}{r} (1 - \eq_i).
    \quad \forall \; \nu, i.
\end{equation}
%
The diagonal elements are still given by \( \shift{\M}^\nu_{ii} = 1 - \sum_{j: j \neq i} \shift{\M}^\nu_{ij} \), but now they are allowed to be negative.
...
\begin{aligned}
  \frac{1}{f^\nu} \pdiff{\CL}{\Mh^\nu_{mn}} = & \,
    \eq_m \prn{ r \brk{\thb_m - \thb_n}
     + \prn{r \mathbf{c}_m + (-1)^\nu + r \lambda} \brk{\etw_m - \etw_n} } \\
     &+ \mu^\nu_{mn} -  \mu^\nu_{mm}
...
\begin{aligned}
  \mu^\nu_{mm} &= - \eq_m \prn{ r \shift{\alb}^\nu_m(s) 
    + \brk{ r \mathbf{c}_m + (-1)^\nu + r \lambda } \shift{\omb}^\nu_m(s)
    }, \\
  \mu^\nu_{mn} &= \eq_m \prn{ 
      r \brk{ \shift{\thb}_n(s) - \shift{\thb}_m(s) - \shift{\alb}^\nu_m(s) }
  % \\ & \phantom{= - \eq_m \big( } 
    + \brk{ r \mathbf{c}_m + (-1)^\nu + r \lambda } 
