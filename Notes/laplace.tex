...
\Cref{eq:KKTz} provides another expression for \(\kktm^\nu_{mm}\).
...
\begin{aligned}
  ...
  \thbm_i - \thbm_j &= \frac{z}{\delta} \sum_k^{j:i} \frac{2k - M}{q_k}, \\
  a_i &= \frac{1}{\eqm_i} 
...
There are only two ways a KKT multiplier can be zero. 
If the sum is empty (\(j = i\)) or it contains exactly one term that is zero (\( j = i + (-1)^\nu \)).
It is impossible for more than one term in the sum to be zero.
Those transitions ddefine the serial topology.
...
\begin{aligned}
  ...
  \varepsilon &= \frac{f_m - \mu g_m 
        \pm \sqrt{(\mu - 1) (g_m - 1) (f_m - \mu) (f_m - g_m)}}
      {1 + f_m -  g_m - \mu} \\
  \implies \qquad
...
\Cref{eq:henvapprox} has a similar form as \cref{eq:envDB}.
