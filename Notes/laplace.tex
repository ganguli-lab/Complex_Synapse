...
\usepackage{a4wide}
% \usepackage{stix2}
\usepackage[tables,graphics,draft,panels=paralist,noeqcref]{sl-preamble}
...
%off-diagonal first passage times
...
As discussed in \cref{sec:recog}, the signal-to-noise ratio is given by
%
...
\begin{aligned}
  \var(\synid\cdt\syn(t))
    ...
    %\\&\hphantom{= \sum_{\alpha}  \av{(\synid^\alpha)^2(\syn^\alpha(t))^2}}
    - \Big(\sum_{\alpha} \av{\synid^\alpha \syn^\alpha(t)} \Big)^2  \\
    &= N\av{1}
...
%   \sum_{j\neq i}\frgm_{ij} + (-1)^\mu \encm_{ij}  &\leq 2rf^\mu, \\
%   \sum_j \frgm_{ij} &=0, &
%   \sum_j \encm_{ij} &=0.
% \end{aligned}
% \end{equation}
...
The area and \(\Delta p\) are invariant under the scale transformation %\cref{eq:scale}
%
...
\begin{equation}\label{eq:serialarea}
 % A = 2\sqrt{N} \sum_k \brk{\prn{k-\frac{n+1}{2}} - \sum_j\prn{j-\frac{n+1}{2}}\eqm_j} \eqm_k \w_k.
...
For any vector \(\mathbf{u}\) that is perpendicular to \(\onev\), we define the following functional
%
\begin{equation*}
    \tau(\mathbf{u}) = - \frac{\mathbf{u}^\dag \mathbf{u}}{\mathbf{u}^\dag \frg \mathbf{u}}
      = \frac{1}{\mathcal{W}(-\frg, \mathbf{u})},
\end{equation*}
%
with the \(\mathbf{u}\) in \( \mathcal{W}(-\frg, \mathbf{u}) \) representing the vector space spanned by \(\mathbf{u}\).

The other quantity of interest in \cref{eq:snrcoeffs} is the amplitude of the eigenmodes.
In analogy, we define the following functionals (for real \(\mathbf{u}\))
%
\begin{equation*}
...
\begin{aligned}
  \CI(\mathbf{u}) &\leq \sqrt{\frac{2\av{\mathbf{u}, -\frg \mathbf{u}}}{r\av{\mathbf{u}, \mathbf{u}}}}
      = \sqrt{2\mathcal{W}(-\frg;\mathbf{u})/r} , &\quad \text{or} \quad
  \CI(\mathbf{u}) \sqrt{r \tau(\mathbf{u})} &\leq \sqrt{2}.
\end{aligned}
\end{equation}
%
...
This model has \(\CI_1 = \tau_1 = 1\), suggesting that there's a factor of \(\sqrt{2}\) looseness in this bound, although we will see below that there is at least one model for which this quantity is higher.
...
\end{equation}
% 
We have shown that the smallest \(\cnst\) for which \cref{eq:gammabnd} is true is at most 2.
...
Thus we see that \(\cnst\) is at least \(\frac{128}{\pi^4} = 1.31 \).
  ...
  \begin{tabular}{BACCCCCC}
    \toprule
      \multicolumn{5}{c}{Domain of validity} & \CI_a & \tau_a 
        & \kktm_\CI & \kktm_a & \kktm_\CA 
        % & \snrb(\tau) 
      \\
    \midrule
      0 &\leq r \tau &\leq \cnst & M &\geq 3 & 1 & \cnst/r 
        & \frac{\cnst(\cnst - r \tau)}{[\cnst + r \tau]^2} & \frac{2\sqrt{\cnst}r\tau}{[\cnst + r \tau]^2} & 0 
        % & \frac{2 \sqrt{N}}{2 + r \tau} 
      \\
      \cnst &< r \tau &< \frac{(M-1)^2}{\cnst} & M &> 3 & \sqrt{\frac{\cnst}{r\tau}} & \tau 
        & 0 & \frac{1}{2\sqrt{r \tau}} & 0 
        % & \sqrt{\frac{N}{2 r \tau}} 
      \\ \addlinespace[0.2ex]
      \frac{(M-1)^2}{\cnst} &\leq r \tau & & M &\geq 3 & \frac{\cnst}{M-1} & \frac{(M-1)^2}{\cnst r} 
        & 0 & \frac{2 \sqrt{\cnst} (M-1)^3}{[(M-1)^2 + \cnst r \tau]^2} 
        & \frac{\cnst (\cnst r \tau - (M-1)^2)}{[(M-1)^2 + 2r \tau]^2} 
        % & \frac{2 \sqrt{N} (M-1)}{(M-1)^2 + 2r \tau} 
      \\
      &-& & M &\leq 3 & 1 & \frac{M - 1}{r} 
        & \frac{(M-1)^2}{[(M-1) + r \tau]^2} %- \frac{\kktm_a \sqrt{\tau_a}}{2} 
        & 0 & \frac{r \tau}{[(M-1) + r \tau]^2} %- \frac{\kktm_a}{2 \sqrt{\tau_a}} 
...
In \cref{fig:envnumth} we compare these theoretical envelopes with the numerical maximisation from \cref{fig:envnum}.
...
\begin{figure}[tb]
  ...
  \Cref{eq:finitelagrangian,eq:KTfinite,eq:finitederivs} are solved numerically with \(f\potdep=\frac{1}{2}\), the number of synapses \(N=1\) and the number of states \(M=10\).
...
\begin{center}
\begin{myenuma}
  \item \label{fig:triangle_pert} \aligntop{%
  \includegraphics[width=0.28\linewidth]{pert_triangle.svg}}
  \item \label{fig:triangle_left} \aligntop{%
  \includegraphics[width=0.28\linewidth]{left_triangle.svg}}
  \item \label{fig:triangle_right} \aligntop{%
  \includegraphics[width=0.28\linewidth]{right_triangle.svg}}
  \\[1em]
  \item \label{fig:triangle_allowed} \aligntop{%
  \includegraphics[width=0.4\linewidth]{allowed_triangle.svg}}
\end{myenuma}
...
\end{equation}
%
Therefore, the only nonzero transitions from \(m\) in \(\W^\nu\) must go to the global minimum of \(\dgn^\nu_m\).
This can also be seen by considering a perturbation that increases \(\Wm^\nu_{mi}\) and decreases \(\Wm^\nu_{mk}\) by the same amount (thus evading the diagonal constraint):
%
\begin{equation}\label{eq:exitminimum}
  \pdiff{\mathcal{L}}{\Wm^\nu_{mi}} - \pdiff{\mathcal{L}}{\Wm^\nu_{mk}}
    = \eqm_m \prn{\dgnm^\nu_{m,k} - \dgnm^\nu_{m,i}} + \kktm^\nu_{mi} - \kktm^\nu_{mk}.
\end{equation}
%
If \( \dgnm^\nu_{m,k} > \dgnm^\nu_{m,i} \) we must have \( \kktm^\nu_{mk} > 0 \) and therefore \( \Wm^\nu_{mk} = 0 \).
...
\begin{center}
\begin{myenuma}
  \item \label{fig:triangle_spacing} \aligntop{ \hspace{-2em}%
  \includegraphics[width=0.4\linewidth]{spacing_triangle.svg}}
  \hspace{0.2em}
  \item \label{fig:scaling_plane}\aligntop{ \hspace{-1em}%
  \includegraphics[width=0.4\linewidth]{scaling_plane.svg}}
\end{myenuma}
\caption[Depicting constraints on topology]{Depicting constraints on topology.
(\ref*{fig:triangle_spacing}) The consequences of \cref{eq:oneexit,eq:doubleexit,eq:twoexits,eq:twodoubleexits,eq:nonexit} in the \( (-\etwm_l, \dgnm_{m,l}) \)-plane, 
...
\subsubsection{Changing planes}\label{sec:planes}

Consider the setup of the previous section.
Suppose there was a nonzero transition in \(\W\pot\) from some state \(p\) to state \(m\).
Now \(m\) must be the minimum of \(\dgn\pot_p\), where \(j\) was the minimum of \(\dgn\pot_m\).

As we move from the \( (-\etwm_l, \dgnm_{m,l}) \)-plane in \cref{fig:triangle_spacing} to the \( (-\etwm_l, \dgnm_{p,l}) \)-plane, the different states will rise or fall depending on \(c_p - c_m\) and the value of \(\etwm_l\).
Let's focus on the rise/fall \emph{relative} to that of \(m\).
If \(c_p > c_m\), states to the left of \(m\) will rise and those to its right will fall relative to it.
If \(c_p < c_m\), states to the left of \(m\) will fall and those to its right will rise relative to it.

If state \(j\) is to the left of \(m\) we must have \(c_p > c_m\).
If \(j\) is to the right, as would be implied by the triangularity of \cref{sec:triangular} if it could be proven more generally,  we must have \(c_p < c_m\).

Similarly, suppose  there was a nonzero transition in \(\W\dep\) from some state \(q\) to state \(m\).
If state \(i\) is to the left of \(m\), as would be implied by the triangularity of \cref{sec:triangular}, we must have \(c_q > c_m\).
If \(i\) is to the right we must have \(c_q < c_m\).

The state \(j\) needs to do more than rise when we change planes.
It must rise further than the gap between the diagonals passing through \(j\) and \(m\), which is \( \frac{\kktm\pot_{mm}}{\eqm_m} \).
Regardless of the signs of the two factors, this rise is given by
%
\begin{equation}\label{eq:plane_rise}
  \prn{c_m - c_p} \prn{\etwm_m - \etwm_j} \geq \frac{\kktm\pot_{mm}}{\eqm_m}.
\end{equation}
%
A similar relation holds between \(c_q\), \(\etwm_i\) and \(\kktm\dep_{mm}\).
...
\Cref{eq:KKTz} provided another expression for \(\kktm^\nu_{mm}\).
...
% (There is still an ambiguity concerning the scale, but it makes sense to choose to saturate the diagonal constraints in \cref{eq:wdof} so that \cref{eq:optmodelscale} holds).
...
\begin{equation*}
  \sum_k^{i:j} f(k) 
    ...
    % = \sgn(j-i) \smashoperator{\sum_{k = \min(i,j)}^{\max(i,j)-1}} f(k)
    = \begin{cases}
...
\begin{figure}[tb]
  ...
  % Requires \usepackage{graphicx}
  \begin{myenuma}
    \item\aligntop{\includegraphics[width=0.26\linewidth]{serial_uni.svg}}\label{fig:serial_uni}
    \hspace{0.02\linewidth}
    \item\aligntop{\includegraphics[width=0.26\linewidth]{serial_sticky.svg}}\label{fig:serial_stk}
    \hspace{0.02\linewidth}
    \item\aligntop{\includegraphics[width=0.26\linewidth]{serial_shorten.svg}}\label{fig:serial_sht}
  \end{myenuma}
  \caption[Semi-empirically optimal models]{Semi-empirical models that maximise memory curve.
...
Looking at the models that were found by numerical maximisation in \cref{fig:envvid} 
...
The two families of models are not identical, but are similar enough to provide some insight.
...
These models' Laplace transforms can be computed analytically, as we will see below, leading to a semi-empirical envelope.
...
\end{equation}
%
This has poles as \( \beta = \frac{(2n-1)\pi\ir}{2m} \).
From \cref{eq:laplaceeig} we see that the poles and residues in the \(s\)-plane are \( (-q_n, \CI_n) \), therefore:
%
\begin{equation}\label{eq:unieig}
  q_n = 1 - \cos \brkfrac{(2n-1)\pi}{2m} 
      % = 2 \sin^2\frac{(2n-1)\pi}{4m},
  \qquad
  \CI_n = \frac{(-1)^{n-1}}{m^2} \cot \brkfrac{(2n-1)\pi}{4m},
  \qquad n = 1,\dots,m.
\end{equation}
%
...
\begin{multline}\label{eq:unienv}
  ...
  \hat{A}_*(s) = \prn{\frac{ \sinh y_* }{ \cosh^2 y_* }} 
      \frac{\operatorname{arccosh}(1+s)}{s},
  \\ \text{where}\quad
...
\end{equation}
%
When \(s \sim \CO(M^{-2}) \ll 1\), we have \(\beta \approx \pm \sqrt{2s}\) and the semi-empirical envelope is approximately
%
...
\begin{aligned}
  ...
  \varepsilon 
    % &= \frac{f_m - \mu g_m 
    %     - \sqrt{(\mu - 1) (g_m - 1) (f_m - \mu) (f_m - g_m)}}
    %   {1 + f_m -  g_m - \mu} \\
    &= \frac{D - S(m\beta)}{D - S((m-1)\beta)} &
  \implies \quad
  \hat{A}(s) 
    % &= \frac{\Delta f_m}{s \Delta m \Delta g_m}
...
\subsection{Semi-empirical envelope}\label{sec:heuristicenv}
...
% using the notation \( \Delta_{\varepsilon}[T(M)] = T(M) - \varepsilon T(M-2) \) for any function \(T(M)\), 
we find
%
...
\begin{aligned}
  ...
  & \frac{\sqrt{N}\brk{S\prn{\tfrac{M\beta(\tau)}{2}} - S\prn{\tfrac{(M-2)\beta(\tau)}{2}}}}
        {\brk{\sqrt{{\frac{M-2}{2} S\prn{\tfrac{M\beta(\tau)}{2}} - \frac{M}{2} S\prn{\tfrac{(M-2)\beta(\tau)}{2}}}} + 1}^2} , &
...
\end{equation}
%
where \(S(\beta) = \cosh \beta - 1\), \( S(\beta(\tau)) = 1/\tau \) (see \cref{eq:detagensol}) and \(y_*\) is defined in \cref{eq:unienv}.
...
\Cref{eq:henvapprox} has a similar form to the theoretical envelope in \cref{eq:envDB}.
...
\begin{figure}[tb]
  ...
  \caption[Semi-empirical envelope for the signal-to-noise ratio]
  {Semi-empirical envelope  for the signal-to-noise ratio, \cref{eq:henv}.
