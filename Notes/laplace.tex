...
This model has \(\CI_1 = \tau_1 = 1\), suggesting that there's a factor of 2 looseness in this bound.
...
This contradicts the requirements of the previous paragraph unless \(M=2\).

The fourth inequality can only be saturated if the outgoing potentitation rates and depression rates separately sum to one for each state.
This does force degenerate states to be lumpable, and we are left with a serial topology with deterministic transitions.
However, this cannot be done for the two end states without violating the requirements for saturating one of the other inequalities.
...
\toprule
  \multicolumn{5}{c}{Conditions} & \CI_a & \tau_a 
    & \mu_\CI & \mu_a & \mu_\CA 
    % & \snrb(\tau) 
  \\
\midrule
  ...
  \\
  &-& & M &\leq 3 & 1 & M - 1 
    & \frac{(M-1)^2}{(M-1) + r \tau} %- \frac{\mu_a \sqrt{M-1}}{2} 
    & 0 & \frac{r \tau}{[(M-1) + r \tau]^2} %- \frac{\mu_a}{2 \sqrt{M-1}} 
