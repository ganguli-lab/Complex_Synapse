...
\begin{equation}\label{eq:pertfe}
  \pdiff{}{\frgm_{ij}} = \frac{1}{2f\pot} \pdiff{}{\MMdm \pot_{ij}} + \frac{1}{2f\dep} \pdiff{}{\MMdm \dep_{ij}},
  \qquad
...
\begin{aligned}
  ...
  \sum_{j\neq i}\frgm_{ij} + (-1)^\mu \encm_{ij}  &\leq 2f^\mu, \\
  \sum_j \frgm_{ij} &=0, &
  \sum_j \encm_{ij} &=0.
\end{aligned}
\end{equation}
...
It shifts \(\frg\) correctly and also shifts \( \shift{\enc}(s) = \enc + \frac{s}{r} \Delta g (\onev \eq - \I) \).
...
The second choice does not leave \(\enc\) invariant (in fact \( \shift{\enc}(s) = \enc + \frac{s}{r} \Delta f (\onev \eq - \I) \)), but because \( \eq (\onev \eq - \I) = 0 \) the shift does not make any contribution to \cref{eq:larea}.
Either way:
%
...
\begin{equation}\label{eq:lareacoeffs}
  \begin{aligned}
    {c}_k(s) &= r\sum_{ij} \eqm_i \encm_{ij} \prn{\fptbm_{ik}(s)-\fptbm_{jk}(s)}, \\
    {a}_j(s) &= r\sum_{k} \encm_{jk} (\etwm_{j}(s) - \etwm_{k}(s)), \\
...
\begin{equation}\label{eq:lAdiff}
  \pdiff{\hat{A}(s)}{\MMdm^\mu_{mn}}
     = f^\mu \eqm_m \prn{ \brk{\thbm_m(s) - \thbm_n(s)}
...
We define the matrix \(\mathbf{X}(s) = r\fund(0) \enc \fund(s)\).
...
This means that in \cite{Lahiri2013synapse} we could take a pair of matrices that violates the ``diagonal'' constraints (second inequality of \eqref{eq:constri}) and use the scale transform \eqref{eq:scale} to construct a pair of matrices that satisfy the ``diagonal'' constraints and have the same area.
...
\begin{myenuma}
  ...
  \item \label{fig:triangle_spacing} \aligntop{%
...
%
\begin{multline}\label{eq:degenerate}
...
\Cref{eq:KKTz} provides another expression for \(\mu^\nu_{mm}\).
...
The area and \(\Delta\pr\) are invariant under the scale transformation \cref{eq:scale}
%
\begin{equation*}%\label{eq:scale}
  \zeta * \MMd \equiv (1-\zeta) \MMd + \zeta \I .
\end{equation*}
...
\begin{equation}\label{eq:serialextarea}
  \CA \leq \frac{(M-1)}{r}\prn{4\eqm_+\eqm_-} = \frac{(M-1)}{r}\prn{1 - (\eqm_+ - \eqm_-)^2}.
\end{equation}
%
