...
\begin{aligned}
  \mu^\nu_{mm} &= - \frac{\eqm_m}{2f^\nu} 
    \left[ 
      c_m(s) (\delta \wm_m + s \delta \etwm_m(s)) - \hat{A}(s)
      + \lambda \delta w_m
      \right. \\ &\phantom{= - \frac{\eqm_m}{2f^\nu} []}\left.
      + (-1)^\nu \prn{ 
        [\encm \thbm(s)]_m - c_m(s) a_m(s)
...
\end{equation}
%
We have not yet ruled the possibility that both \( \MMdm\pot_{mn} \) and \( \MMdm\dep_{mn} \) are nonzero.
...
(\ref*{fig:triangle_pert}) The type of perturbation considered in \cref{eq:pertKupdn}.
...
Both \cref{eq:twoexits,eq:twodoubleexits} require degeneracy.
