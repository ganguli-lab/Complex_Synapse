...
\begin{multline}\label{eq:unienv}
  \alpha = \exp\prn{\pm\frac{2y_*}{M_*}},
  \quad
  s = 2\sinh^2\frac{y_*}{M_*},
  \qquad
  \hat{A}_*(s) = \prn{\frac{ 2\sinh^2 \frac{y_*}{2} }{ y_* \cosh y_* } } \frac{\abs{\log\alpha(s)}}{s},
  \\ \text{where}\quad
...
\end{equation}
%
When $s \sim \CO(M^{-2}) \ll 1$, we have $\alpha \approx 1 \pm \sqrt{2s}$ and the heuristic envelope is approximately
%
...
\begin{equation}\label{eq:shortenenc}
  \begin{aligned}
  \enc_{M-1M} = -\enc_{21} &= \frac{(1-\varepsilon)}{2},
  &\qquad
  \eq_1 = \eq_M &= \frac{(1-\varepsilon)}{M-2\varepsilon},
  \\ &&
  \eq_i &=  \frac{1}{M-2\varepsilon}
  \quad\text{for}\quad 1<i<M.
  \end{aligned}
\end{equation}
%
...
\begin{equation}\label{eq:shortenareaeta}
  \hat{A}(s) = \frac{(1-\varepsilon)(\etw_1(s)-\etw_M(s)) + \varepsilon(\etw_2(s)-\etw_{M-1}(s))}{M-2\varepsilon}
       = \frac{2(1-\varepsilon) \etw_1(s) + 2\varepsilon\,\etw_2(s)}{M-2\varepsilon}.
\end{equation}
%
...
\begin{aligned}
  ...
  \delta\eta_1 &= \frac{2\alpha^{\frac{M}{2}}}
         {[(1-\varepsilon)(\alpha^{M}+1) + \varepsilon(\alpha^2-\alpha+1)(\alpha^{M-2}+1)]s}, \\
  B &= - \frac{2\alpha^{\frac{M}{2}}(\alpha^2-\alpha+1 + (1-\varepsilon)(\alpha-1))}
         {(1+\alpha)[(1-\varepsilon)(\alpha^{M}+1) + \varepsilon(\alpha^2-\alpha+1)(\alpha^{M-2}+1)]s}, \\
  C &= - \frac{2\alpha^{\frac{M-2}{2}}(\alpha^2-\alpha+1 - (1-\varepsilon)\alpha(\alpha-1))}
         {(1+\alpha)[(1-\varepsilon)(\alpha^{M}+1) + \varepsilon(\alpha^2-\alpha+1)(\alpha^{M-2}+1)]s}, \\
\end{aligned}
\end{equation}
%
...
\begin{equation}\label{eq:shortenlaplace}
  \hat{A}(s) = \frac{2 [(1-\varepsilon)(\alpha^{\frac{M}{2}}-1)^2
          + \varepsilon(\alpha^2-\alpha+1)(\alpha^{\frac{M-2}{2}}-1)^2] }
          { [M-2(1-\varepsilon)]
            [(1-\varepsilon)(\alpha^{M}+1) + \varepsilon(\alpha^2-\alpha+1)(\alpha^{M-2}+1)]s }.
\end{equation}
%
This interpolates between \eqref{eq:unilaplace} at $\varepsilon=0$ and the same expression for $M-2$ at $\varepsilon=1$.
This is easier to see if we define
%
\begin{equation}\label{eq:shortdefs}
\begin{aligned}
  F_M(\alpha) &= (\alpha^{\frac{M}{2}} - 1)^2, &
  \Delta F_M(\alpha) &= F_M(\alpha) - \zeta F_{M-2}(\alpha), &
  f_m(\alpha) = \frac{F_M(\alpha)}{\Delta F_M(\alpha)}, \\
  G_M(\alpha) &= \alpha^{M} + 1, &
  \Delta G_M(\alpha) &= G_M(\alpha) - \zeta G_{M-2}(\alpha), &
  g_m(\alpha) = \frac{G_M(\alpha)}{\Delta G_M(\alpha)}, \\
  \text{where}\quad \zeta &= \alpha^2 - \alpha + 1, &
  m &= \frac{M}{2}. \\
\end{aligned}
\end{equation}
%
Then the uniform model \eqref{eq:unilaplace} has \( \hat{A}(s) = \frac{F_m(\alpha)}{s m G_m(\alpha)} \).
Then \cref{eq:shortenlaplace} can be be written as
%
\begin{equation}\label{eq:shortsimple}
  \hat{A}(s) =
    \frac{(1-\varepsilon)F_m(\alpha) + \varepsilon \zeta F_{m-1}(\alpha)}
         {s (m - \varepsilon) ((1-\varepsilon)G_m(\alpha) + \varepsilon \zeta G_{m-1}(\alpha))}
    % = \frac{F_m - \varepsilon \Delta F_{m}}
    %      {s (m - \varepsilon) (G_m - \varepsilon \Delta G_{m})}
    = \brk{\frac{\Delta F_{m}}{s \Delta G_{m}}}
    \frac{f_m - \varepsilon}{(m - \varepsilon) (g_m - \varepsilon)}.
\end{equation}
%
...
The dotted orange line is the extension of the \(M=4\) line with \(\varepsilon\) held at 0, as discussed below \cref{eq:shortenvalid}.
\label{fig:shorten}}
\end{center}
\end{figure}
...
\begin{equation}\label{eq:shortopt}
  ...
  \qquad
  \hat{A}(s) = \brk{\frac{\Delta F_{m}}{s \Delta G_{m}}}
    \frac{1}{\brk{\sqrt{\abs{m - f_m}} - \sqrt{\abs{g_m - f_m}}}^2}.
\end{equation}
%
...
\begin{aligned}
  \left.\pdiff{\hat{A}(s)}{\varepsilon}\right|_{\varepsilon=0} \propto &\,
    M(\alpha^2-\alpha+1)(\alpha-1)\alpha^{\frac{M-2}{2}}(\alpha^{M-1}-1) - (\alpha^{\frac{M}{2}}-1)^2(\alpha^{M}+1)
      = 0,\\
  \left.\pdiff{\hat{A}(s)}{\varepsilon}\right|_{\varepsilon=1} \propto &\,
    (M-2)(\alpha-1)\alpha^{\frac{M-2}{2}}(\alpha^{M-1}-1) - (\alpha^2-\alpha+1)(\alpha^{\frac{M-2}{2}}-1)^2(\alpha^{M-2}+1)
      = 0,\\
    \text{where}\quad
    s = &\, \frac{(\alpha-1)^2}{2\alpha},
\end{aligned}
...
This is depicted by the dotted orange line in \cref{fig:shorten}\ref{fig:short_ratio}.
...
In this case, however, there is an overlap as seen in \cref{fig:shorten}\ref{fig:short_ratio}.
The \(M=2\) region of validity overlaps everything else.
The other overlaps are less extreme, but they do exist.
