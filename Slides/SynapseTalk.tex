...
%
 ...
 \begin{overlayarea}{10cm}{2cm}
  ...
  %
  \note[item]{only constrains SNR at that time, not others}
  %
  \only<2>{Another time}
  %
  \note[item]{get another bound}
  %
  \only<3>{All times $\to$ envelope}
  %
  \note[item]{vary time of max. no curve can cross this. is it tight? can any constrained set of exps be acheived?}
  %
